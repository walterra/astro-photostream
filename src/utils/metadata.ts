...
export abstract class LLMAnalyzer {
  ...
  abstract analyzeContent(imagePath: string, exifData?: ExifData): Promise<LLMAnalysis>;
  ...
  protected getFallbackAnalysis(imagePath: string, exifData?: ExifData): LLMAnalysis {
...
export class ClaudeAnalyzer extends LLMAnalyzer {
  ...
  async analyzeContent(imagePath: string, exifData?: ExifData): Promise<LLMAnalysis> {
    const filename = path.basename(imagePath);
    ...
    try {
      ...
      const recentContext = this.recentOutputs.length > 0
        ? `Recent outputs to avoid repetition: ${this.recentOutputs.slice(-3).map(r => r.title).join(', ')}`
        : '';
      ...
      const prompt = this.options.prompt || `You are analyzing a photograph for a photography blog. 
