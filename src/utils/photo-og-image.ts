...
const getOgOptions = async (): Promise<SatoriOptions> => {
  try {
    ...
    const possiblePaths = [
      ...
      path.resolve(
        process.cwd(),
        'node_modules/astro-photostream/src/assets/fonts'
      ),
    ...
    for (const fontsDir of possiblePaths) {
      try {
        robotoRegular = fs.readFileSync(
          path.join(fontsDir, 'roboto-mono-regular.ttf')
        );
        robotoBold = fs.readFileSync(
          path.join(fontsDir, 'roboto-mono-700.ttf')
        );
        break;
      } catch {
        // Try next path
...
export const renderPhoto = (
  photo: { src: string; alt: string } | null | undefined,
  index: number
) => {
  if (photo && photo.src) {
...
export const createPhotoGridMarkup = (
  title: string,
  subtitle: string,
  details: string,
  ...
  const photoSlots: Array<{ src: string; alt: string } | null> =
    Array(9).fill(null);
...
export const processPhotoImages = async (
  ...
  for (let i = 0; i < Math.min(photos.length, maxPhotos); i++) {
    ...
    try {
      ...
      if (
        photo &&
        photo.data.coverImage.src &&
        typeof photo.data.coverImage.src === 'object' &&
        'src' in photo.data.coverImage.src
      ) {
        ...
        if (
          ...
          (processedSrc.startsWith('data:') ||
            processedSrc.startsWith('/_astro/'))
        ) {
        ...
        console.error(
          'Invalid coverImage structure for photo:',
          photo?.id,
      ...
      if (useProcessedImage && imageSrc.startsWith('/_astro/')) {
        ...
        try {
          ...
          console.error(
            `Failed to process production image: ${distPath}`,
            distError
          );
...
export const generateOgImage = async (
  ...
  const svg = await satori(
    createPhotoGridMarkup(title, subtitle, details, photos),
    options
  );
